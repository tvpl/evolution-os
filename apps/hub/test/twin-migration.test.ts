...
beforeAll(async () => {
  pool = await freshDb("evoos_test_twin_migration");
});
...
describe("migration 003 (twin)", () => {
  it("applies from zero creating snapshots and candidates, and is idempotent", async () => {
  ...
  it("seedDevGrants grants twin.read and candidate.decide to both dev tenants", async () => {
    ...
    for (const orgId of ["org_dev_a", "org_dev_b"]) {
      ...
      expect(caps).toContain("twin.read");
      expect(caps).toContain("candidate.decide");
    }
  });
});
