...
beforeAll(async () => {
  pool = await freshDb("evoos_test_portfolio_migration");
});
...
describe("migration 009 (portfolio)", () => {
  it("applies from zero creating the portfolio campaign tables, and is idempotent", async () => {
    const tables = await pool.query(
      "select table_name from information_schema.tables where table_schema = 'public' order by table_name",
    );
    const names = tables.rows.map((r: { table_name: string }) => r.table_name);
    for (const required of ["project_relations", "campaigns", "campaign_waves", "campaign_items"]) {
  ...
  it("seedDevGrants grants portfolio.write to both dev tenants", async () => {
    ...
    for (const orgId of ["org_dev_a", "org_dev_b"]) {
      ...
      expect(caps).toContain("portfolio.write");
    }
  });
});
