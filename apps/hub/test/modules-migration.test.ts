...
beforeAll(async () => {
  pool = await freshDb("evoos_test_modules_migration");
});
...
describe("migration 008 (modules)", () => {
  it("applies from zero creating the module lifecycle tables, and is idempotent", async () => {
    const tables = await pool.query(
      "select table_name from information_schema.tables where table_schema = 'public' order by table_name",
    );
    const names = tables.rows.map((r: { table_name: string }) => r.table_name);
    for (const required of ["modules", "module_publisher_keys", "module_versions", "module_installations"]) {
  ...
  it("seedDevGrants grants module.write to both dev tenants", async () => {
    ...
    for (const orgId of ["org_dev_a", "org_dev_b"]) {
      ...
      expect(caps).toContain("module.write");
    }
  });
});
