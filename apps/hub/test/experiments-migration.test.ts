...
beforeAll(async () => {
  pool = await freshDb("evoos_test_experiments_migration");
});
...
describe("migration 005 (experiments)", () => {
  it("applies from zero creating experiments/experiment_artifacts, and is idempotent", async () => {
    const tables = await pool.query(
      "select table_name from information_schema.tables where table_schema = 'public' order by table_name",
    );
    const names = tables.rows.map((r: { table_name: string }) => r.table_name);
    for (const required of ["experiments", "experiment_artifacts"]) {
  ...
  it("seedDevGrants grants experiment.write to both dev tenants", async () => {
    ...
    for (const orgId of ["org_dev_a", "org_dev_b"]) {
      ...
      expect(caps).toContain("experiment.write");
    }
  });
});
