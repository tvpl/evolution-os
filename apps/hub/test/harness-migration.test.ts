...
beforeAll(async () => {
  pool = await freshDb("evoos_test_harness_migration");
});
...
describe("migration 007 (harness)", () => {
  it("applies from zero creating the harness tables, and is idempotent", async () => {
    const tables = await pool.query(
      "select table_name from information_schema.tables where table_schema = 'public' order by table_name",
    );
    const names = tables.rows.map((r: { table_name: string }) => r.table_name);
    for (const required of ["harness_inventories", "harness_eval_cases", "harness_eval_runs"]) {
  ...
  it("seedDevGrants grants harness.write to both dev tenants", async () => {
    ...
    for (const orgId of ["org_dev_a", "org_dev_b"]) {
      ...
      expect(caps).toContain("harness.write");
    }
  });
});
