...
| Pergunta | Resposta |
| --- | --- |
| Usuário entende o valor? | Sim — uma proposal `readyForReview` só chega a `adopted`/fechada depois de passar por um experimento formal: 2 variantes declaradas, plano de verificação fixado ANTES do resultado, prova anexada (artifacts), veredito calculado deterministicamente contra o plano, e a decisão de outcome preservada — "o sistema prova antes de recomendar adoção" (build-sequence, Slice 4) |
| O novo artifact está no knowledge model? | Sim — o `proposal_digest` do experimento é o primeiro exemplo real de "prova do que foi de fato aprovado" (proposal spec §5: "approval invalid if proposal/plan digest changes"), computado via `canonicalJson` sobre o que está persistido, não sobre o que o cliente alega. O veredito (`hypothesis_met`/`hypothesis_not_met`/`inconclusive`) é sempre decomposto — nunca um "funcionou"/"não funcionou" opaco (EVO-FR-012) |
| Evidence/decision lineage existe? | Sim, estendido: `proposal` → `experiment` (digest fixado) → `experiment_artifacts` (prova anexada) → `evaluate` (veredito determinístico contra o plano) → `close` → `decision` (`subject_id = proposal.id`, mesmo mecanismo do Slice 1/3). O experimento em si vira um elo auditável entre a proposta e a decisão final, sem substituir nenhum lineage já existente |
| Policy e classification cobrem o fluxo? | Sim — `experiment.write` segue o mesmo deny-by-default dos Slices 0-3, testado para os dois tenants dev na mesma edição (evitando o bug de grant single-tenant pego no Slice 3 T1). Nenhuma capability nova além dessa: attach de artifact, evaluate e close reusam a mesma `experiment.write` — nenhum requisito pediu granularidade maior |
| Failure/retry/idempotency definidos? | Sim — `startExperiment` roda em uma única transação (leitura da proposal + insert do experimento + transição de status), igual ao padrão `withTx` dos Slices 1-3; anexar o mesmo proof artifact duas vezes é idempotente (`ON CONFLICT DO NOTHING`, mesmo padrão do dedup de signals); avaliar/fechar um experimento fora do status esperado é 409 determinístico, nunca corrompe estado |
| Evals incluem negative cases? | Sim: variantes != 2, plano de verificação incompleto, proposal fora de `readyForReview`, artifact de outro projeto, avaliação sem o campo obrigatório, valor não numérico e não-null, avaliação/fechamento fora do status esperado, experimento/proposal inexistente, cross-tenant em toda rota nova |
| O profile Lite continua possível? | Sim — nenhuma infraestrutura nova além do mesmo Postgres; `evaluateExperiment` é 100% determinístico e local, sem chamada de rede/LLM, preservando o profile Lite. O "sandbox" é um campo `environment` declarado (metadado), nunca uma execução real — ver Out of Scope da spec |
| Alguma hipótese do ecossistema foi invalidada? | Não invalidou nenhum ADR. Confirmou que a extração de `canonicalJson` (antes privada no Cartographer) era necessária além do Slice 2 — o mesmo problema de round-trip de jsonb do Postgres se repetiria no digest da proposal se a função fosse duplicada em vez de compartilhada; a extração byte-a-byte com a suíte de regressão do Slice 2 rodando neste slice provou que a mudança foi segura |
